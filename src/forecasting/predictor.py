...
def _alert_level(pct: float) -> str:
    ...
    if pct >= 60:  return "advisory"
    return "normal"


def forecast_budget(route_id: str,
    ...
    if len(daily_emissions) < 7:
        logger.warning(f"Route {route_id}: fewer than 7 data points — forecast less reliable")
    ...
    if current_pct >= 100:
        ...
        breach_prob = round(min(1.0, 0.5 + (100 - current_pct) / 200 * (slope / 0.01 + 1)), 2)
    else:
        breach_prob = round(max(0.0, current_pct / 100 * slope * 0.1), 2)

    projected_end = current_pct + float(budget_pct[-1]) if len(budget_pct) > 0 else current_pct
    ...
    summary = (
        ...
        if breach_day else
        f"On track — projected {projected_end:.1f}% of monthly budget at 30 days."
    )
